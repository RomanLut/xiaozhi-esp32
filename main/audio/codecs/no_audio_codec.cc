...
// NoAudioCodecPdmWithRef implementation
NoAudioCodecPdmWithRef::NoAudioCodecPdmWithRef(int input_sample_rate, int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout, gpio_num_t mic_sck, gpio_num_t mic_din) 
    : NoAudioCodecPdmWithRef(input_sample_rate, output_sample_rate, spk_bclk, spk_ws, spk_dout, I2S_STD_SLOT_LEFT, mic_sck, mic_din) {
}

NoAudioCodecPdmWithRef::NoAudioCodecPdmWithRef(int input_sample_rate, int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout, i2s_std_slot_mask_t spk_slot_mask, gpio_num_t mic_sck, gpio_num_t mic_din) {
    duplex_ = true;
    input_reference_ = true;
    input_sample_rate_ = input_sample_rate;
    output_sample_rate_ = output_sample_rate;
    input_channels_ = 2;  // Mic + Reference

    // For AEC to work properly, input and output sample rates must be equal
    if (input_sample_rate_ != output_sample_rate_) {
        ESP_LOGW(TAG, "AEC requires equal sample rates! Forcing output_rate = input_rate (%dHz)", input_sample_rate_);
        output_sample_rate_ = input_sample_rate_;
    }

    // Allocate reference buffer (approximately 100ms at input rate)
    ref_buffer_.resize(input_sample_rate_ / 10);

    // Create TX channel for speaker (standard I2S)
    i2s_chan_config_t tx_chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG((i2s_port_t)1, I2S_ROLE_MASTER);
    tx_chan_cfg.dma_desc_num = AUDIO_CODEC_DMA_DESC_NUM;
    tx_chan_cfg.dma_frame_num = AUDIO_CODEC_DMA_FRAME_NUM;
    tx_chan_cfg.auto_clear_after_cb = true;
    ...
    ESP_LOGE(TAG, "PDM is not supported");
#endif
    ESP_LOGI(TAG, "PdmWithRef channels created");
}

int NoAudioCodecPdmWithRef::Write(const int16_t* data, int samples) {
    std::lock_guard<std::mutex> lock(data_if_mutex_);
    
    // Prepare volume-adjusted buffer
    std::vector<int32_t> buffer(samples);
    int32_t volume_factor = pow(double(output_volume_) / 100.0, 2) * 65536;
    for (int i = 0; i < samples; i++) {
    ...
    for (int i = 0; i < samples; i++) {
        ref_buffer_[write_pos_] = data[i];
        write_pos_ = (write_pos_ + 1) % ref_buffer_.size();
    }
    return bytes_written / sizeof(int32_t);
}

int NoAudioCodecPdmWithRef::Read(int16_t* dest, int samples) {
    // We need to read half of the requested samples because we are converting mono to stereo (Mic + Ref)
    int samples_to_read = samples / 2;
    std::vector<int16_t> mic_buffer(samples_to_read);
    size_t bytes_read;
    
    if (i2s_channel_read(rx_handle_, mic_buffer.data(), samples_to_read * sizeof(int16_t), &bytes_read, portMAX_DELAY) != ESP_OK) {
        ESP_LOGE(TAG, "Read Failed!");
        return 0;
    }
    ...
    // Interleave mic and reference data (2 channels for AFE)
    std::lock_guard<std::mutex> ref_lock(ref_mutex_);
    for (int i = 0; i < read_samples; i++) {
    ...
    return read_samples * 2;  // Return samples for 2 channels
}
